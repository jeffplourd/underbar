(function() {
  ...
  _.map = function(collection, iterator) {
    ...
    //Using 'each' here allows us to apply the 'iterator' function to each element in the collection
    //and push the result to 'mappedArr'
    var mappedArr = [];

    _.each(collection, function(elem, index, collection) {
      mappedArr.push(iterator(elem, index, collection));
    });
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, function(accumulator, elem) {
      if(iterator(elem) && accumulator === true) {
        return true;
      }else {
        return false;
      }
  ...
  _.extend = function(obj) {
    for(var i = 1, len = arguments.length; i < len; i++) {
      var objToAdd = arguments[i];
      for(var val in objToAdd) {
        obj[val] = objToAdd[val];
      }
    }
    return obj;
  };
  ...
  _.shuffle = function(array) {
    var myArray = array.slice(), newArray = [];
